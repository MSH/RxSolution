...
procedure TForm1.Open(const prmConnectionString : string);
begin
...
procedure TForm1.qryDosingBeforePost(DataSet: TDataSet);
var
  prvWghtLo: Double;
  prvWghtHi: Double;
  prvAgeHi: Double;
...
with DataSet do
  ...
  if (FieldByName('freeForm_str').AsString = '') or (FieldByName('freeForm_str').IsNull) then
    ...
    TPrescription.RxDescription(prvBrandName, prvGenericName, prvDoseUnit, prvRoute,
  ...
  FieldByName('descriptionRange_Str').AsString :=
    TPrescription.RxDoseRangeDescription(prvAgeLo, prvAgeHi, prvWghtLo, prvWghtHi);
...
procedure TForm1.qryDosingNewRecord(DataSet: TDataSet);
begin
...
procedure TForm1.RzDBCheckBox1Click(Sender: TObject);
begin
  UpdateEccentricDisplay( (Sender as TRzDBCheckBox).Checked );
end;

procedure TForm1.SetDbConnectionString(const Value: string);
begin
...
procedure TForm1.UpdateEccentricDisplay(prmDisplay : boolean = True);
var
  prvTop  : integer;
  prvSpace: integer;
begin
