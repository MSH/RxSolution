object PrescriptionListFrm: TPrescriptionListFrm
  Left = 219
  Top = 34
  Width = 1598
  Height = 955
  Caption = 'PrescriptionListFrm'
  Color = clBtnFace
  Font.Charset = ANSI_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  OnCreate = FormCreate
  OnKeyPress = FormKeyPress
  OnResize = FormResize
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 16
  object pnl_Main_Back: TPanel
    Left = 0
    Top = 0
    Width = 1582
    Height = 917
    Align = alClient
    BevelOuter = bvNone
    Caption = 'f'
    Color = clWhite
    TabOrder = 0
    DesignSize = (
      1582
      917)
    object pnlToolbar_Header_ShapeBorder: TShape
      Left = 10
      Top = 6
      Width = 1429
      Height = 84
      Brush.Style = bsClear
      Pen.Color = 4227072
      Pen.Style = psInsideFrame
    end
    object pnl_Image_Header: TRzLabel
      Left = 15
      Top = 11
      Width = 1864
      Height = 58
      Anchors = [akLeft, akTop, akRight]
      AutoSize = False
      Caption = 'Prescriptions'
      Color = 10998215
      Font.Charset = ANSI_CHARSET
      Font.Color = clWhite
      Font.Height = -38
      Font.Name = 'Nina'
      Font.Style = [fsBold]
      ParentColor = False
      ParentFont = False
      Layout = tlCenter
    end
    object pnl_Main_PatientListPanel: TPanel
      Left = 14
      Top = 98
      Width = 432
      Height = 711
      BevelOuter = bvNone
      Color = clWhite
      Font.Charset = ANSI_CHARSET
      Font.Color = clSilver
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      object Label83: TLabel
        Left = 0
        Top = 36
        Width = 432
        Height = 22
        Align = alTop
        Alignment = taCenter
        AutoSize = False
        Caption = 'Patient Manager'
        Color = 10998215
        Font.Charset = ANSI_CHARSET
        Font.Color = clWhite
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
      end
      object rzbGradientA: TRzBackground
        Left = 0
        Top = 57
        Width = 432
        Height = 700
        Active = True
        Align = alNone
        GradientColorStart = 12639959
        GradientColorStop = clWhite
        ImageStyle = isCenter
        ShowGradient = True
        ShowImage = False
        ShowTexture = False
      end
      object Shape2: TShape
        Left = 0
        Top = 58
        Width = 432
        Height = 1
        Align = alTop
        Pen.Color = clBtnHighlight
      end
      object Label1: TLabel
        Left = 10
        Top = 59
        Width = 57
        Height = 16
        Caption = 'Patients'
        Font.Charset = ANSI_CHARSET
        Font.Color = clNavy
        Font.Height = -15
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
        Transparent = True
      end
      object rzsLine01: TRzSeparator
        Left = 6
        Top = 79
        Width = 283
        HighlightColor = clYellow
        ShowGradient = True
        Color = clBtnFace
        ParentColor = False
      end
      object Label2: TLabel
        Left = 7
        Top = 84
        Width = 175
        Height = 26
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'Last Name'
        Color = clInfoBk
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlue
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentColor = False
        ParentFont = False
      end
      object Label3: TLabel
        Left = 7
        Top = 138
        Width = 175
        Height = 26
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'IPN #'
        Color = clInfoBk
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlue
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentColor = False
        ParentFont = False
      end
      object Label4: TLabel
        Left = 7
        Top = 111
        Width = 175
        Height = 26
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'First name'
        Color = clInfoBk
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlue
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentColor = False
        ParentFont = False
      end
      object Label17: TLabel
        Left = 7
        Top = 165
        Width = 175
        Height = 26
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'National ID #'
        Color = clInfoBk
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlue
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentColor = False
        ParentFont = False
      end
      object Label23: TLabel
        Left = 7
        Top = 219
        Width = 175
        Height = 20
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'Active?'
        Color = clInfoBk
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlue
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentColor = False
        ParentFont = False
      end
      object Label10: TLabel
        Left = 7
        Top = 192
        Width = 175
        Height = 26
        Alignment = taRightJustify
        AutoSize = False
        Caption = 'PAS #'
        Color = clInfoBk
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlue
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentColor = False
        ParentFont = False
      end
      object Label5: TLabel
        Left = 103
        Top = 6
        Width = 31
        Height = 16
        Caption = 'From'
      end
      object lblstatus: TLabel
        Left = 4
        Top = 256
        Width = 5
        Height = 16
        Caption = '*'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Height = -15
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        Transparent = True
      end
      object lblInterfaceResult: TLabel
        Left = 4
        Top = 241
        Width = 5
        Height = 16
        Caption = '*'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Height = -15
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        Transparent = True
      end
      object Panel2: TPanel
        Left = 0
        Top = 0
        Width = 432
        Height = 36
        Align = alTop
        BevelOuter = bvNone
        Color = clWhite
        TabOrder = 0
        object Shape4: TShape
          Left = 0
          Top = 34
          Width = 432
          Height = 2
          Align = alBottom
          Pen.Color = clHighlight
        end
        object ElSpeedButton19: TElSpeedButton
          Left = 69
          Top = 0
          Width = 34
          Height = 34
          PullDownBtn = mbLeft
          PullDownInterval = 0
          PullDownEnabled = True
          Flat = True
          NormalImage.Data = {
            
            BCAAAABCCBD3D4D4D4D4D4D4D4D4}
          DisabledImage.Data = {
            }
          MouseInImage.Data = {
            
            57535357A6E2E4E4E4E4E4E4E4E4}
          PressedImage.Data = {
            
            FFFFFFFFFFFFFFFFFFFFFEF9CF8E8ECFF9FEFFFFFFFFFFFFFFFF}
          AutoSize = False
          TransparentColor = clBlack
          Align = alLeft
          Color = clBtnFace
          ParentColor = False
          Action = atnPatientDisplayColumns
          DockOrientation = doNoOrient
        end
        object ElSpeedButton1: TElSpeedButton
          Left = 0
          Top = 0
          Width = 34
          Height = 34
          PullDownBtn = mbLeft
          Flat = True
          NormalImage.Data = {
            
            FFFFFFFFFFFFFFFFFFFFFDE6D1BAAED2E6FAFDFEFEFEFFFFFFFF}
          DisabledImage.Data = {
            
            7E7E7E7D7D7D7D776E67676E777D7E7E7D7E7E7E7E7E}
          MouseInImage.Data = {
            
            FEFEFEFEFEFEFEFEFCFBC4604949494960C0C2F9FEFEFEFEFEFF}
          PressedImage.Data = {
            
            FFFFFFFEFEFEFEFEFEFEF9D76560638CD7ECF7FCFEFEFEFFFFFF}
          AutoSize = False
          TransparentColor = clBlack
          Align = alLeft
          Color = clBtnFace
          ParentColor = False
          Action = atnPerson_Add
          DockOrientation = doNoOrient
        end
        object ElSpeedButton9: TElSpeedButton
          Left = 34
          Top = 0
          Width = 35
          Height = 34
          PullDownBtn = mbLeft
          PullDownInterval = 0
          PullDownEnabled = True
          Flat = True
          NormalImage.Data = {
            
            AEAEAEAEAEAEADA697888897A6ADAEAEAEAEAEAEAEAE}
          DisabledImage.Data = {
            
            5959}
          MouseInImage.Data = {
            
            B9B9}
          PressedImage.Data = {
            
            EDEDECE2AE6767AEE2ECEDEDEDEDEDEDEDED}
          AutoSize = False
          TransparentColor = clBlack
          Align = alLeft
          Color = clBtnFace
          ParentColor = False
          Action = atnPerson_Edit
          DockOrientation = doNoOrient
        end
        object btnSearchPaab: TRzButton
          Left = 138
          Top = 0
          Width = 92
          Height = 31
          Alignment = taLeftJustify
          Caption = 'Search Paab'
          Color = clWhite
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -15
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentColor = False
          ParentFont = False
          TabOrder = 0
          Visible = False
          OnClick = btnSearchPaabClick
        end
      end
      object grdPatientList: TdxDBGrid
        Left = 10
        Top = 278
        Width = 415
        Height = 396
        Bands = <
          item
          end>
        DefaultLayout = False
        HeaderPanelRowCount = 1
        KeyField = 'person_ID'
        SummaryGroups = <>
        SummarySeparator = ', '
        BorderStyle = bsNone
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Height = -15
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        PopupMenu = pmnuPatient
        TabOrder = 1
        OnDblClick = atnPerson_EditExecute
        BandFont.Charset = ANSI_CHARSET
        BandFont.Color = clWindowText
        BandFont.Height = -11
        BandFont.Name = 'MS Sans Serif'
        BandFont.Style = []
        DataSource = dsqryPatientList
        Filter.Criteria = {00000000}
        HeaderColor = clInfoBk
        HeaderFont.Charset = ANSI_CHARSET
        HeaderFont.Color = clBlack
        HeaderFont.Height = -12
        HeaderFont.Name = 'Tahoma'
        HeaderFont.Style = [fsBold]
        LookAndFeel = lfFlat
        OptionsBehavior = [edgoAnsiSort, edgoAutoSort, edgoCaseInsensitive, edgoDragScroll, edgoEditing, edgoEnterShowEditor, edgoImmediateEditor, edgoMultiSelect, edgoMultiSort, edgoStoreToRegistry, edgoTabThrough, edgoVertThrough]
        OptionsCustomize = [edgoBandMoving, edgoBandSizing, edgoColumnMoving, edgoColumnSizing, edgoHeaderPanelSizing]
        OptionsDB = [edgoCancelOnExit, edgoCanNavigation, edgoConfirmDelete, edgoLoadAllRecords, edgoUseBookmarks]
        OptionsView = [edgoAutoWidth, edgoBandHeaderWidth, edgoRowSelect, edgoUseBitmap]
        PreviewFont.Charset = ANSI_CHARSET
        PreviewFont.Color = clBlue
        PreviewFont.Height = -11
        PreviewFont.Name = 'MS Sans Serif'
        PreviewFont.Style = []
        RegistryPath = '\Software\RxSolution\1.2\Grids\Rx\grdPatientList'
        object grdPatientListpersonLastName_Str: TdxDBGridColumn
          Caption = 'Last name'
          DisableEditor = True
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'MS Sans Serif'
          Font.Style = [fsBold]
          Width = 35
          BandIndex = 0
          RowIndex = 0
          FieldName = 'personLastName_Str'
          DisableFilter = True
        end
        object grdPatientListpersonFirstName_Str: TdxDBGridColumn
          Caption = 'First name'
          DisableEditor = True
          Width = 36
          BandIndex = 0
          RowIndex = 0
          FieldName = 'personFirstName_Str'
          DisableFilter = True
        end
        object grdPatientListpersonGender_Str: TdxDBGridColumn
          Caption = 'M/F'
          DisableEditor = True
          Width = 20
          BandIndex = 0
          RowIndex = 0
          FieldName = 'personGender_Str'
          DisableFilter = True
        end
        object grdPatientListpersonIDNumber_Str: TdxDBGridColumn
          Caption = 'ID'
          DisableEditor = True
          Width = 20
          BandIndex = 0
          RowIndex = 0
          FieldName = 'personIDNumber_Str'
          DisableFilter = True
        end
        object grdPatientListpersonAgeYears_int: TdxDBGridMaskColumn
          Caption = 'Age'
          DisableEditor = True
          Width = 20
          BandIndex = 0
          RowIndex = 0
          FieldName = 'AgeYears'
          DisableFilter = True
        end
        object grdPatientListpersonInstitution_str: TdxDBGridColumn
          Caption = 'Clinic'
          DisableEditor = True
          Width = 28
          BandIndex = 0
          RowIndex = 0
          FieldName = 'personInstitution_str'
          DisableFilter = True
        end
        object grdPatientListpersonRefNoCurrent_str: TdxDBGridColumn
          Caption = '#'
          DisableEditor = True
          Width = 20
          BandIndex = 0
          RowIndex = 0
          FieldName = 'personRefNoCurrent_str'
          DisableFilter = True
        end
        object grdPatientListpersonTelNoCurrent_str: TdxDBGridColumn
          Caption = 'Phone'
          DisableEditor = True
          Width = 28
          BandIndex = 0
          RowIndex = 0
          FieldName = 'personTelNoCurrent_str'
          DisableFilter = True
        end
        object grdPatientListpersonCelNoCurrent_str: TdxDBGridColumn
          Caption = 'Cell'
          DisableEditor = True
          Width = 21
          BandIndex = 0
          RowIndex = 0
          FieldName = 'personCelNoCurrent_str'
          DisableFilter = True
        end
        object grdPatientListpersonDOB_Dat: TdxDBGridColumn
          Caption = 'DOB'
          DisableEditor = True
          BandIndex = 0
          RowIndex = 0
          FieldName = 'personDOB_Dat'
        end
      end
      object RzEdit1: TRzEdit
        Tag = 1
        Left = 183
        Top = 84
        Width = 152
        Height = 24
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Height = -15
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        FrameController = rzcMain
        FramingPreference = fpCustomFraming
        ParentFont = False
        TabOnEnter = True
        TabOrder = 2
        OnChange = RzEdit1Change
      end
      object RzEdit2: TRzEdit
        Left = 183
        Top = 138
        Width = 152
        Height = 24
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Height = -15
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        FrameController = rzcMain
        FramingPreference = fpCustomFraming
        ParentFont = False
        TabOnEnter = True
        TabOrder = 4
        OnChange = RzEdit2Change
      end
      object RzEdit3: TRzEdit
        Tag = 2
        Left = 183
        Top = 111
        Width = 152
        Height = 24
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Height = -15
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        FrameController = rzcMain
        FramingPreference = fpCustomFraming
        ParentFont = False
        TabOnEnter = True
        TabOrder = 3
        OnChange = RzEdit3Change
      end
      object RzEdit4: TRzEdit
        Tag = 3
        Left = 183
        Top = 165
        Width = 152
        Height = 24
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Height = -15
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        FrameController = rzcMain
        FramingPreference = fpCustomFraming
        ParentFont = False
        TabOnEnter = True
        TabOrder = 5
        OnChange = RzEdit4Change
      end
      object RzEdit5: TRzEdit
        Tag = 4
        Left = 183
        Top = 192
        Width = 152
        Height = 24
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Height = -15
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        FrameController = rzcMain
        FramingPreference = fpCustomFraming
        ParentFont = False
        TabOnEnter = True
        TabOrder = 6
        OnChange = RzEdit5Change
      end
      object Button1: TButton
        Left = 184
        Top = 245
        Width = 151
        Height = 27
        Action = atnFindPatient
        TabOrder = 8
      end
      object cmbPatientActive: TComboBox
        Left = 183
        Top = 218
        Width = 151
        Height = 24
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Height = -15
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ItemHeight = 16
        ParentFont = False
        TabOrder = 7
        Text = '<All>'
        Items.Strings = (
          '<All>'
          'True'
          'False')
      end
    end
    object pnl_Main_PrescriptionPanel: TPanel
      Left = 447
      Top = 98
      Width = 1093
      Height = 858
      BevelOuter = bvNone
      Font.Charset = ANSI_CHARSET
      Font.Color = 16744576
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      object rzbGradientB: TRzBackground
        Left = 0
        Top = 82
        Width = 690
        Height = 697
        Active = True
        Align = alNone
        GradientColorStart = 12639959
        GradientColorStop = clWhite
        ImageStyle = isCenter
        ShowGradient = True
        ShowImage = False
        ShowTexture = False
      end
      object rzLblRxList: TLabel
        Left = 0
        Top = 59
        Width = 690
        Height = 22
        Alignment = taCenter
        AutoSize = False
        Caption = 'Prescriptions'
        Color = 10998215
        Font.Charset = ANSI_CHARSET
        Font.Color = clWhite
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
      end
      object Shape3: TShape
        Left = 0
        Top = 58
        Width = 1093
        Height = 1
        Align = alTop
        Pen.Color = clBtnHighlight
      end
      object Label6: TLabel
        Left = 34
        Top = 6
        Width = 31
        Height = 16
        Caption = 'From'
      end
      object Label7: TLabel
        Left = 219
        Top = 6
        Width = 17
        Height = 16
        Caption = 'To'
      end
      object Label11: TLabel
        Left = 0
        Top = 36
        Width = 1093
        Height = 22
        Align = alTop
        Alignment = taCenter
        AutoSize = False
        Caption = 'List of Associated Prescriptions'
        Color = 10998215
        Font.Charset = ANSI_CHARSET
        Font.Color = clWhite
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
      end
      object rzLblRxListItems: TLabel
        Left = 693
        Top = 59
        Width = 400
        Height = 22
        Alignment = taCenter
        AutoSize = False
        Caption = 'Prescription Items'
        Color = 10998215
        Font.Charset = ANSI_CHARSET
        Font.Color = clWhite
        Font.Height = -17
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentColor = False
        ParentFont = False
      end
      object Panel3: TPanel
        Left = 0
        Top = 0
        Width = 1093
        Height = 36
        Align = alTop
        BevelOuter = bvNone
        Color = clWhite
        TabOrder = 0
        object Shape1: TShape
          Left = 0
          Top = 34
          Width = 1093
          Height = 2
          Align = alBottom
          Pen.Color = clHighlight
        end
        object ElSpeedButton2: TElSpeedButton
          Left = 0
          Top = 0
          Width = 34
          Height = 34
          PullDownBtn = mbLeft
          Flat = True
          NormalImage.Data = {
            
            FFFFFFFFFFFFFFFFFFFFFDE6D1BAAED2E6FAFDFEFEFEFFFFFFFF}
          DisabledImage.Data = {
            
            7E7E7E7D7D7D7D776E67676E777D7E7E7D7E7E7E7E7E}
          MouseInImage.Data = {
            
            FEFEFEFEFEFEFEFEFCFBC4604949494960C0C2F9FEFEFEFEFEFF}
          PressedImage.Data = {
            
            FFFFFFFEFEFEFEFEFEFEF9D76560638CD7ECF7FCFEFEFEFFFFFF}
          AutoSize = False
          TransparentColor = clBlack
          Align = alLeft
          Color = clBtnFace
          ParentColor = False
          Action = atnPrescription_Add
          DockOrientation = doNoOrient
        end
        object ElSpeedButton3: TElSpeedButton
          Left = 34
          Top = 0
          Width = 35
          Height = 34
          PullDownBtn = mbLeft
          PullDownInterval = 0
          PullDownEnabled = True
          Flat = True
          NormalImage.Data = {
            
            AEAEAEAEAEAEADA697888897A6ADAEAEAEAEAEAEAEAE}
          DisabledImage.Data = {
            
            5959}
          MouseInImage.Data = {
            
            B9B9}
          PressedImage.Data = {
            
            EDEDECE2AE6767AEE2ECEDEDEDEDEDEDEDED}
          AutoSize = False
          TransparentColor = clBlack
          Align = alLeft
          Color = clBtnFace
          ParentColor = False
          Action = atnPrescription_Edit
          DockOrientation = doNoOrient
        end
        object ElSpeedButton4: TElSpeedButton
          Left = 69
          Top = 0
          Width = 34
          Height = 34
          PullDownBtn = mbLeft
          PullDownInterval = 0
          PullDownEnabled = True
          Flat = True
          NormalImage.Data = {
            
            BCAAAABCCBD3D4D4D4D4D4D4D4D4}
          DisabledImage.Data = {
            }
          MouseInImage.Data = {
            
            57535357A6E2E4E4E4E4E4E4E4E4}
          PressedImage.Data = {
            
            FFFFFFFFFFFFFFFFFFFFFEF9CF8E8ECFF9FEFFFFFFFFFFFFFFFF}
          AutoSize = False
          TransparentColor = clBlack
          Align = alLeft
          Color = clBtnFace
          ParentColor = False
          Action = atnPrescriptionDisplayColumns
          DockOrientation = doNoOrient
        end
        object Label8: TLabel
          Left = 103
          Top = 6
          Width = 31
          Height = 16
          Caption = 'From'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -15
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object Label9: TLabel
          Left = 288
          Top = 6
          Width = 17
          Height = 16
          Caption = 'To'
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -15
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
        end
        object lblRxStatus: TLabel
          Left = 737
          Top = 10
          Width = 3
          Height = 16
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -15
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          Transparent = True
        end
        object RzButton1: TRzButton
          Left = 473
          Top = 1
          Width = 257
          Height = 31
          Caption = 'View Prescriptions'
          Color = clWhite
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -15
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentColor = False
          ParentFont = False
          TabOrder = 0
          OnClick = RzButton1Click
        end
        object rzStartDate: TRzDateTimeEdit
          Left = 139
          Top = 6
          Width = 139
          Height = 24
          CalendarColors.Days = clWindowText
          CalendarColors.FillDays = clBtnShadow
          CalendarColors.DaysOfWeek = clWindowText
          CalendarColors.Lines = clBtnShadow
          CalendarColors.SelectedDateBack = clHighlight
          CalendarColors.SelectedDateFore = clHighlightText
          CalendarColors.TodaysDateFrame = clMaroon
          ClockFaceColors.Face = clBtnFace
          ClockFaceColors.Hands = clWindowText
          ClockFaceColors.Numbers = clWindowText
          ClockFaceColors.HourTicks = clBtnShadow
          ClockFaceColors.MinuteTicks = clWindowText
          EditType = etDate
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -15
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 1
        end
        object rzEndDate: TRzDateTimeEdit
          Left = 315
          Top = 5
          Width = 149
          Height = 24
          CalendarColors.Days = clWindowText
          CalendarColors.FillDays = clBtnShadow
          CalendarColors.DaysOfWeek = clWindowText
          CalendarColors.Lines = clBtnShadow
          CalendarColors.SelectedDateBack = clHighlight
          CalendarColors.SelectedDateFore = clHighlightText
          CalendarColors.TodaysDateFrame = clMaroon
          ClockFaceColors.Face = clBtnFace
          ClockFaceColors.Hands = clWindowText
          ClockFaceColors.Numbers = clWindowText
          ClockFaceColors.HourTicks = clBtnShadow
          ClockFaceColors.MinuteTicks = clWindowText
          EditType = etDate
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -15
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 2
        end
      end
      object rzgRxListItems: TRzGroupBox
        Left = 694
        Top = 82
        Width = 399
        Height = 706
        Color = 12639959
        Font.Charset = ANSI_CHARSET
        Font.Color = clRed
        Font.Height = -15
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 1
        object grdPrescriptionItemsActive: TdxDBGrid
          Left = 1
          Top = 17
          Width = 397
          Height = 401
          Bands = <
            item
            end>
          DefaultLayout = False
          HeaderPanelRowCount = 1
          KeyField = 'RXItem_ID'
          SummaryGroups = <>
          SummarySeparator = ', '
          Align = alTop
          BorderStyle = bsNone
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -15
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 0
          BandFont.Charset = ANSI_CHARSET
          BandFont.Color = clWindowText
          BandFont.Height = -11
          BandFont.Name = 'MS Sans Serif'
          BandFont.Style = []
          DataSource = dsqryPrescriptionItemActive
          Filter.Criteria = {00000000}
          HeaderColor = clInfoBk
          HeaderFont.Charset = ANSI_CHARSET
          HeaderFont.Color = clBlack
          HeaderFont.Height = -12
          HeaderFont.Name = 'Tahoma'
          HeaderFont.Style = [fsBold]
          LookAndFeel = lfFlat
          OptionsBehavior = [edgoAutoSort, edgoCaseInsensitive, edgoDragScroll, edgoEditing, edgoEnterShowEditor, edgoImmediateEditor, edgoTabThrough, edgoVertThrough]
          OptionsCustomize = [edgoBandMoving, edgoBandSizing, edgoColumnMoving, edgoColumnSizing, edgoHeaderPanelSizing]
          OptionsDB = [edgoUseBookmarks]
          OptionsView = [edgoAutoWidth, edgoBandHeaderWidth, edgoRowAutoHeight, edgoRowSelect, edgoUseBitmap]
          PreviewFont.Charset = ANSI_CHARSET
          PreviewFont.Color = clBlue
          PreviewFont.Height = -11
          PreviewFont.Name = 'MS Sans Serif'
          PreviewFont.Style = []
          object grdPrescriptionItemsActiveDspDescription_Str: TdxDBGridColumn
            Caption = 'Item'
            DisableEditor = True
            BandIndex = 0
            RowIndex = 0
            FieldName = 'DspDescription_Str'
          end
        end
        object dxDBGrid1: TdxDBGrid
          Left = 1
          Top = 418
          Width = 397
          Height = 284
          Bands = <
            item
            end>
          DefaultLayout = False
          HeaderPanelRowCount = 1
          SummaryGroups = <>
          SummarySeparator = ', '
          Align = alTop
          BorderStyle = bsNone
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -15
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 1
          BandFont.Charset = ANSI_CHARSET
          BandFont.Color = clWindowText
          BandFont.Height = -11
          BandFont.Name = 'MS Sans Serif'
          BandFont.Style = []
          DataSource = dsItemHistory
          Filter.Criteria = {00000000}
          HeaderColor = clInfoBk
          HeaderFont.Charset = ANSI_CHARSET
          HeaderFont.Color = clBlack
          HeaderFont.Height = -12
          HeaderFont.Name = 'Tahoma'
          HeaderFont.Style = [fsBold]
          LookAndFeel = lfFlat
          OptionsBehavior = [edgoAutoSort, edgoCaseInsensitive, edgoDragScroll, edgoEditing, edgoEnterShowEditor, edgoImmediateEditor, edgoTabThrough, edgoVertThrough]
          OptionsCustomize = [edgoBandMoving, edgoBandSizing, edgoColumnMoving, edgoColumnSizing, edgoHeaderPanelSizing]
          OptionsDB = [edgoUseBookmarks]
          OptionsView = [edgoAutoWidth, edgoBandHeaderWidth, edgoRowSelect, edgoUseBitmap]
          PreviewFont.Charset = ANSI_CHARSET
          PreviewFont.Color = clBlue
          PreviewFont.Height = -11
          PreviewFont.Name = 'MS Sans Serif'
          PreviewFont.Style = []
          Anchors = [akLeft, akTop, akRight, akBottom]
          object dxDBGrid1FrmFormulation_Str: TdxDBGridColumn
            Caption = 'History'
            BandIndex = 0
            RowIndex = 0
            FieldName = 'FrmFormulation_Str'
          end
        end
      end
      object rzgRxList: TRzGroupBox
        Left = 0
        Top = 80
        Width = 689
        Height = 634
        Color = 10998215
        Font.Charset = ANSI_CHARSET
        Font.Color = clRed
        Font.Height = -15
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 2
        object grdPrescriptionActive: TdxDBGrid
          Left = 1
          Top = 17
          Width = 687
          Height = 316
          Bands = <
            item
            end>
          DefaultLayout = False
          HeaderPanelMaxRowCount = 2
          HeaderPanelRowCount = 1
          KeyField = 'prescription_ID'
          SummaryGroups = <>
          SummarySeparator = ', '
          Align = alTop
          BorderStyle = bsNone
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -15
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          PopupMenu = pmnuRX
          TabOrder = 0
          OnDblClick = atnPrescription_EditExecute
          BandFont.Charset = ANSI_CHARSET
          BandFont.Color = clWindowText
          BandFont.Height = -11
          BandFont.Name = 'MS Sans Serif'
          BandFont.Style = []
          DataSource = dsqryPrescriptionActive
          Filter.Active = True
          Filter.Criteria = {00000000}
          HeaderColor = clInfoBk
          HeaderFont.Charset = ANSI_CHARSET
          HeaderFont.Color = clBlack
          HeaderFont.Height = -12
          HeaderFont.Name = 'Tahoma'
          HeaderFont.Style = [fsBold]
          LookAndFeel = lfFlat
          OptionsBehavior = [edgoAutoSort, edgoCaseInsensitive, edgoDragScroll, edgoEditing, edgoEnterShowEditor, edgoImmediateEditor, edgoStoreToRegistry, edgoTabThrough, edgoVertThrough]
          OptionsCustomize = [edgoBandMoving, edgoBandPanelSizing, edgoBandSizing, edgoColumnMoving, edgoColumnSizing, edgoHeaderPanelSizing]
          OptionsDB = [edgoCancelOnExit, edgoCanNavigation, edgoLoadAllRecords, edgoUseBookmarks]
          OptionsView = [edgoAutoWidth, edgoBandHeaderWidth, edgoRowSelect, edgoUseBitmap]
          PreviewFont.Charset = ANSI_CHARSET
          PreviewFont.Color = clBlue
          PreviewFont.Height = -11
          PreviewFont.Name = 'MS Sans Serif'
          PreviewFont.Style = []
          RegistryPath = '\Software\RxSolution\1.2\Grids\Rx\grdPrescriptionActive'
          Anchors = [akLeft, akTop, akRight, akBottom]
          object grdPrescriptionActiveRxDate_dat: TdxDBGridDateColumn
            Caption = 'Date'
            DisableEditor = True
            Width = 75
            BandIndex = 0
            RowIndex = 0
            FieldName = 'RxDate_dat'
          end
          object grdPrescriptionActiveprescriptionNumber_Str: TdxDBGridColumn
            Caption = 'Rx Number'
            DisableEditor = True
            Width = 61
            BandIndex = 0
            RowIndex = 0
            FieldName = 'prescriptionNumber_Str'
            DisableFilter = True
          end
          object grdPrescriptionActiveprescriberName_Str: TdxDBGridColumn
            Caption = 'Prescriber'
            DisableEditor = True
            Width = 82
            BandIndex = 0
            RowIndex = 0
            FieldName = 'prescriberName_Str'
          end
          object grdPrescriptionActiveprescriptionCost_Mon: TdxDBGridCurrencyColumn
            Caption = 'Cost'
            DisableEditor = True
            Width = 40
            BandIndex = 0
            RowIndex = 0
            FieldName = 'prescriptionCost_Mon'
            Nullable = False
            DisableFilter = True
          end
          object grdPrescriptionActiveprescriptionStatus_Str: TdxDBGridColumn
            Caption = 'Status'
            DisableEditor = True
            Width = 32
            BandIndex = 0
            RowIndex = 0
            FieldName = 'prescriptionStatus_Str'
          end
          object grdPrescriptionActiveRepeatStatus_Str: TdxDBGridColumn
            Caption = 'Repeats'
            DisableEditor = True
            Width = 30
            BandIndex = 0
            RowIndex = 0
            FieldName = 'RepeatStatus_Str'
          end
          object grdPrescriptionActivePosted_bol: TdxDBGridCheckColumn
            Caption = 'Posted'
            DisableEditor = True
            MinWidth = 20
            Width = 53
            BandIndex = 0
            RowIndex = 0
            FieldName = 'dispensingPosted_bol'
            ValueChecked = 'True'
            ValueUnchecked = 'False'
          end
          object grdPrescriptionActivevstVisitDate_dat: TdxDBGridColumn
            Caption = 'Visit Date'
            DisableEditor = True
            Visible = False
            Width = 68
            BandIndex = 0
            RowIndex = 0
            FieldName = 'vstVisitDate_dat'
          end
          object grdPrescriptionActiveDemanderName_Str: TdxDBGridColumn
            Caption = 'Demander'
            DisableEditor = True
            Width = 185
            BandIndex = 0
            RowIndex = 0
            FieldName = 'DemanderName_Str'
          end
          object grdPrescriptionActivePosted_dat: TdxDBGridColumn
            Caption = 'Posted Date'
            DisableEditor = True
            BandIndex = 0
            RowIndex = 0
            FieldName = 'Posted_dat'
          end
        end
        object dxDBGrid2: TdxDBGrid
          Left = 1
          Top = 333
          Width = 687
          Height = 177
          Bands = <
            item
            end>
          DefaultLayout = False
          HeaderPanelRowCount = 1
          KeyField = 'Protocol'
          SummaryGroups = <>
          SummarySeparator = ', '
          Align = alTop
          BorderStyle = bsNone
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -17
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 1
          BandFont.Charset = ANSI_CHARSET
          BandFont.Color = clWindowText
          BandFont.Height = -11
          BandFont.Name = 'MS Sans Serif'
          BandFont.Style = []
          DataSource = dsPatientProtocols
          Filter.Active = True
          Filter.CaseInsensitive = True
          Filter.Criteria = {00000000}
          HeaderColor = clInfoBk
          HeaderFont.Charset = ANSI_CHARSET
          HeaderFont.Color = clBlack
          HeaderFont.Height = -13
          HeaderFont.Name = 'Tahoma'
          HeaderFont.Style = [fsBold]
          LookAndFeel = lfFlat
          OptionsBehavior = [edgoAutoSort, edgoCaseInsensitive, edgoDragScroll, edgoEditing, edgoEnterShowEditor, edgoImmediateEditor, edgoMultiSort, edgoTabThrough, edgoVertThrough]
          OptionsCustomize = [edgoBandMoving, edgoBandSizing, edgoColumnMoving, edgoColumnSizing, edgoHeaderPanelSizing]
          OptionsDB = [edgoLoadAllRecords, edgoUseBookmarks, edgoUseLocate]
          OptionsView = [edgoAutoWidth, edgoBandHeaderWidth, edgoRowSelect, edgoUseBitmap]
          PreviewFont.Charset = ANSI_CHARSET
          PreviewFont.Color = clBlue
          PreviewFont.Height = -11
          PreviewFont.Name = 'MS Sans Serif'
          PreviewFont.Style = []
          RegistryPath = '\Software\RxSolution\1.2\Grids\Rx\PatientProtocols'
          object dxDBGrid2Date_dat: TdxDBGridDateColumn
            Caption = 'Date'
            DisableEditor = True
            Width = 60
            BandIndex = 0
            RowIndex = 0
            FieldName = 'Date_dat'
            DateButtons = []
            DisableFilter = True
          end
          object dxDBGrid2Protocol: TdxDBGridColumn
            DisableEditor = True
            Width = 108
            BandIndex = 0
            RowIndex = 0
            FieldName = 'Protocol'
            DisableFilter = True
          end
          object dxDBGrid2Indication_str: TdxDBGridColumn
            DisableEditor = True
            Width = 169
            BandIndex = 0
            RowIndex = 0
            FieldName = 'Indication'
            DisableFilter = True
          end
          object dxDBGrid2Status: TdxDBGridColumn
            DisableEditor = True
            Width = 204
            BandIndex = 0
            RowIndex = 0
            FieldName = 'Status'
          end
        end
        object dxDBGrid4: TdxDBGrid
          Left = 1
          Top = 510
          Width = 687
          Height = 121
          Bands = <
            item
            end>
          DefaultLayout = False
          HeaderPanelRowCount = 1
          KeyField = 'AllergyDescription_str'
          SummaryGroups = <>
          SummarySeparator = ', '
          Align = alTop
          BorderStyle = bsNone
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -17
          Font.Name = 'MS Sans Serif'
          Font.Style = []
          ParentFont = False
          TabOrder = 2
          BandFont.Charset = ANSI_CHARSET
          BandFont.Color = clWindowText
          BandFont.Height = -11
          BandFont.Name = 'MS Sans Serif'
          BandFont.Style = []
          DataSource = dsPersoAllergies
          Filter.Criteria = {00000000}
          HeaderColor = clInfoBk
          HeaderFont.Charset = ANSI_CHARSET
          HeaderFont.Color = clRed
          HeaderFont.Height = -13
          HeaderFont.Name = 'Tahoma'
          HeaderFont.Style = [fsBold]
          LookAndFeel = lfFlat
          OptionsBehavior = [edgoAutoSort, edgoCaseInsensitive, edgoDragScroll, edgoEditing, edgoEnterShowEditor, edgoImmediateEditor, edgoTabThrough, edgoVertThrough]
          OptionsCustomize = [edgoBandMoving, edgoBandSizing, edgoColumnMoving, edgoColumnSizing, edgoHeaderPanelSizing]
          OptionsDB = [edgoUseBookmarks]
          OptionsView = [edgoAutoWidth, edgoBandHeaderWidth, edgoRowSelect, edgoUseBitmap]
          PreviewFont.Charset = ANSI_CHARSET
          PreviewFont.Color = clBlue
          PreviewFont.Height = -11
          PreviewFont.Name = 'MS Sans Serif'
          PreviewFont.Style = []
          object dxDBGrid4AllergyDescription_str: TdxDBGridColumn
            Caption = 'Allergies'
            BandIndex = 0
            RowIndex = 0
            FieldName = 'AllergyDescription_str'
          end
          object dxDBGrid4personAllergySource_Str: TdxDBGridColumn
            Caption = 'Source'
            BandIndex = 0
            RowIndex = 0
            FieldName = 'personAllergySource_Str'
          end
        end
      end
    end
  end
  object ActionList1: TActionList
    Left = 160
    Top = 16
    object atnPrescription_Edit: TAction
      Caption = 'Edit Rx...'
      Enabled = False
      Hint = 'Edit currently selected prescription.'
      OnExecute = atnPrescription_EditExecute
    end
    object atnPrescription_Add: TAction
      Caption = 'Add Rx...'
      Enabled = False
      Hint = 'Add a new prescription for currently selected patient.'
      OnExecute = atnPrescription_AddExecute
    end
    object atnPrescription_Delete: TAction
      Caption = 'Delete Rx...'
      Enabled = False
      Hint = 'Delete currently selected prescription.'
      OnExecute = atnPrescription_DeleteExecute
    end
    object atnPerson_Add: TAction
      Caption = 'Add patient...'
      Hint = 'Add a new patient.'
      OnExecute = atnPerson_AddExecute
    end
    object atnPerson_Save: TAction
      Caption = 'Save patient...'
      Enabled = False
      Hint = 'Save existing patient information'
    end
    object atnPerson_Edit: TAction
      Caption = 'Edit patient...'
      Enabled = False
      Hint = 'Edit current patient.'
      OnExecute = atnPerson_EditExecute
    end
    object atnPrescription_EditInactive: TAction
      Caption = 'View inactive Rx...'
      Enabled = False
      Hint = 'View an inactive or posted Rx.'
    end
    object atnPerson_Update: TAction
      Caption = 'Update (All)'
    end
    object atnPersonSearch: TAction
      Caption = 'Search...'
      ShortCut = 16455
    end
    object atnPatientDisplayColumns: TAction
      Caption = 'atnPatientDisplayColumns'
      OnExecute = atnPatientDisplayColumnsExecute
    end
    object atnPrescriptionDisplayColumns: TAction
      Caption = 'atnPrescriptionDisplayColumns'
      OnExecute = atnPrescriptionDisplayColumnsExecute
    end
    object atnPrescription_Copy: TAction
      Caption = 'Copy Rx...'
      Enabled = False
      OnExecute = atnPrescription_CopyExecute
    end
    object atnFindPatient: TAction
      Caption = 'Search'
      OnExecute = atnFindPatientExecute
    end
  end
  object conPatientPrescription: TADOConnection
    CommandTimeout = 120
    ConnectionString = 
      'Provider=SQLOLEDB.1;Password=rxsolution;Persist Security Info=Tr' +
      'ue;User ID=sa;Initial Catalog=RxBarCoding;Data Source=.\sqlexpre' +
      'ss'
    ConnectionTimeout = 120
    LoginPrompt = False
    Provider = 'SQLOLEDB.1'
    Left = 192
    Top = 16
  end
  object qryPatientList: TADOQuery
    Connection = conPatientPrescription
    CursorType = ctStatic
    AfterEdit = qryPatientListAfterEdit
    AfterPost = qryPatientListAfterPost
    AfterScroll = qryPatientListAfterScroll
    Parameters = <>
    SQL.Strings = (
      'SELECT    *'
      'FROM view_v01_Patient_List'
      ''
      '')
    Left = 256
    Top = 16
  end
  object dsqryPatientList: TDataSource
    DataSet = qryPatientList
    Left = 288
    Top = 16
  end
  object dsqryPrescriptionActive: TDataSource
    DataSet = qryPrescriptionActive
    Left = 288
    Top = 48
  end
  object qryPrescriptionActive: TADOQuery
    Connection = conPatientPrescription
    CursorType = ctStatic
    DataSource = dsqryPatientList
    Parameters = <
      item
        Name = 'person_ID'
        Attributes = [paNullable]
        DataType = ftGuid
        NumericScale = 255
        Precision = 255
        Size = 16
        Value = '{7A6D040D-2CF6-434C-B906-0012815E273B}'
      end
      item
        Name = '@StartDate'
        Attributes = [paNullable]
        DataType = ftDateTime
        NumericScale = 3
        Precision = 23
        Size = 16
        Value = Null
      end
      item
        Name = '@EndDate'
        Attributes = [paNullable]
        DataType = ftDateTime
        NumericScale = 3
        Precision = 23
        Size = 16
        Value = Null
      end>
    SQL.Strings = (
      'SELECT     * '
      'FROM         view_v01_Prescription_List'
      
        'WHERE    (prescriptionActive_bol = 1) AND (patient_ID = :person_' +
        'ID) '
      'AND (vstTreamentPrescribed_bol = 1)   '
      'AND (RxDate_dat >=:@StartDate)'
      'AND (RxDate_dat <=:@EndDate)')
    Left = 256
    Top = 48
    object qryPrescriptionActiveRxDate_dat: TDateTimeField
      FieldName = 'RxDate_dat'
      ReadOnly = True
    end
    object qryPrescriptionActiveRxDateRepeat_Dat: TDateTimeField
      FieldName = 'RxDateRepeat_Dat'
      ReadOnly = True
    end
    object qryPrescriptionActiveprescriptionNumber_Str: TWideStringField
      FieldName = 'prescriptionNumber_Str'
      Size = 50
    end
    object qryPrescriptionActiverepeats_Int: TIntegerField
      FieldName = 'repeats_Int'
    end
    object qryPrescriptionActiveprescriptionCost_Mon: TBCDField
      FieldName = 'prescriptionCost_Mon'
      Precision = 19
    end
    object qryPrescriptionActivepatient_ID: TGuidField
      FieldName = 'patient_ID'
      FixedChar = True
      Size = 38
    end
    object qryPrescriptionActiveprescription_ID: TGuidField
      FieldName = 'prescription_ID'
      FixedChar = True
      Size = 38
    end
    object qryPrescriptionActivepatientName_Str: TWideStringField
      FieldName = 'patientName_Str'
      Size = 50
    end
    object qryPrescriptionActiveprescriber_ID: TGuidField
      FieldName = 'prescriber_ID'
      FixedChar = True
      Size = 38
    end
    object qryPrescriptionActiveprescriberName_Str: TWideStringField
      FieldName = 'prescriberName_Str'
      Size = 50
    end
    object qryPrescriptionActiveprescriberRXLevel_Str: TWideStringField
      FieldName = 'prescriberRXLevel_Str'
      Size = 50
    end
    object qryPrescriptionActiveprescriptionStatus_Str: TWideStringField
      FieldName = 'prescriptionStatus_Str'
      Size = 100
    end
    object qryPrescriptionActiveprescriptionActive_bol: TBooleanField
      FieldName = 'prescriptionActive_bol'
    end
    object qryPrescriptionActivedispensingPosted_bol: TBooleanField
      FieldName = 'dispensingPosted_bol'
    end
    object qryPrescriptionActivefolderNumber_Str: TWideStringField
      FieldName = 'folderNumber_Str'
      Size = 50
    end
    object qryPrescriptionActivedispenserName_Str: TWideStringField
      FieldName = 'dispenserName_Str'
      Size = 50
    end
    object qryPrescriptionActivedispenserRxLevel_Str: TWideStringField
      FieldName = 'dispenserRxLevel_Str'
      Size = 50
    end
    object qryPrescriptionActiveRepeatStatus_Str: TWideStringField
      FieldName = 'RepeatStatus_Str'
      Size = 250
    end
    object qryPrescriptionActivepersonpregnant_bol: TBooleanField
      FieldName = 'personpregnant_bol'
    end
    object qryPrescriptionActivevstTreamentPrescribed_bol: TBooleanField
      FieldName = 'vstTreamentPrescribed_bol'
    end
    object qryPrescriptionActivevstVisitDate_dat: TDateTimeField
      FieldName = 'vstVisitDate_dat'
    end
    object qryPrescriptionActiveRxCancelReasonsAfterPosting_str: TWideStringField
      FieldName = 'RxCancelReasonsAfterPosting_str'
      Size = 50
    end
    object qryPrescriptionActiveDemanderName_Str: TWideStringField
      FieldName = 'DemanderName_Str'
      Size = 50
    end
    object qryPrescriptionActivePosted_dat: TDateTimeField
      FieldName = 'Posted_dat'
    end
    object qryPrescriptionActivePostedBy_str: TWideStringField
      FieldName = 'PostedBy_str'
      Size = 30
    end
  end
  object qryPrescriptionItemActive: TADOQuery
    Connection = conPatientPrescription
    CursorType = ctStatic
    DataSource = dsqryPrescriptionActive
    Parameters = <
      item
        Name = 'prescription_ID'
        DataType = ftGuid
        NumericScale = 255
        Precision = 255
        Size = 16
        Value = '{334CA81A-1DCE-4FB8-8863-12C0AAD5A88E}'
      end>
    SQL.Strings = (
      
        'SELECT     dbo.TblRXItem.RXItem_ID, dbo.TblRXItem.DspDescription' +
        '_Str'
      'FROM         dbo.TblPrescription INNER JOIN'
      
        '                      dbo.TblRXItem ON dbo.TblPrescription.presc' +
        'ription_ID = dbo.TblRXItem.RX_ID'
      'WHERE dbo.TblPrescription.prescription_ID=:prescription_ID')
    Left = 256
    Top = 80
    object qryPrescriptionItemActiveRXItem_ID: TGuidField
      FieldName = 'RXItem_ID'
      FixedChar = True
      Size = 38
    end
    object qryPrescriptionItemActiveDspDescription_Str: TWideStringField
      FieldName = 'DspDescription_Str'
      Size = 300
    end
  end
  object dsqryPrescriptionItemActive: TDataSource
    DataSet = qryPrescriptionItemActive
    Left = 288
    Top = 80
  end
  object pmnuRX: TPopupMenu
    Left = 608
    Top = 16
    object Add1: TMenuItem
      Action = atnPrescription_Add
    end
    object Edit1: TMenuItem
      Action = atnPrescription_Edit
    end
    object CopyRx1: TMenuItem
      Action = atnPrescription_Copy
    end
  end
  object qryPersonPositionActive: TADOQuery
    Connection = conPatientPrescription
    CursorType = ctStatic
    AfterEdit = qryPersonPositionActiveAfterEdit
    AfterPost = qryPersonPositionActiveAfterPost
    DataSource = dsqryPatientList
    Parameters = <
      item
        Name = 'person_ID'
        Attributes = [paNullable]
        DataType = ftGuid
        NumericScale = 255
        Precision = 255
        Size = 16
        Value = Null
      end>
    SQL.Strings = (
      
        'select * from tblPersonPosition where (person_ID = :person_ID) a' +
        'nd (personPositionCurrent_bol = 1)')
    Left = 480
    Top = 56
  end
  object dsqryPersonPositionActive: TDataSource
    DataSet = qryPersonPositionActive
    Left = 520
    Top = 56
  end
  object rzcMain: TRzFrameController
    FrameVisible = True
    FramingPreference = fpCustomFraming
    Left = 128
    Top = 16
  end
  object pmnuInactiveRx: TPopupMenu
    Left = 344
    Top = 80
    object ViewinactiveRx1: TMenuItem
      Action = atnPrescription_EditInactive
    end
  end
  object pmnuPatient: TPopupMenu
    Left = 728
    Top = 8
    object atnPrescriptionItemEdit1: TMenuItem
      Action = atnPerson_Add
    end
    object Editpatient1: TMenuItem
      Action = atnPerson_Edit
    end
  end
  object rzvFileLaunchUtil: TRzLauncher
    Action = 'Open'
    Timeout = -1
    Left = 480
    Top = 24
  end
  object qryItemHistory: TADOQuery
    Connection = conPatientPrescription
    CursorType = ctStatic
    DataSource = dsqryPatientList
    Parameters = <
      item
        Name = 'Person_ID'
        Attributes = [paNullable]
        DataType = ftGuid
        NumericScale = 255
        Precision = 255
        Size = 38
        Value = '{7A6D040D-2CF6-434C-B906-0012815E273B}'
      end>
    SQL.Strings = (
      'SELECT DISTINCT dbo.TblRXItem.FrmFormulation_Str'
      'FROM         dbo.TblRXItem INNER JOIN'
      
        '                      dbo.TblPrescription ON dbo.TblRXItem.RX_ID' +
        ' = dbo.TblPrescription.prescription_ID'
      'Where  dbo.TblPrescription.Patient_ID=:Person_ID')
    Left = 256
    Top = 112
  end
  object dsItemHistory: TDataSource
    DataSet = qryItemHistory
    Left = 288
    Top = 112
  end
  object dsPersoAllergies: TDataSource
    DataSet = qryPersonAllergies
    Left = 336
    Top = 160
  end
  object qryPersonAllergies: TADOQuery
    Connection = conPatientPrescription
    CursorType = ctStatic
    DataSource = dsqryPatientList
    Parameters = <
      item
        Name = 'person_ID'
        Attributes = [paNullable]
        DataType = ftGuid
        NumericScale = 255
        Precision = 255
        Size = 16
        Value = Null
      end>
    SQL.Strings = (
      
        'SELECT  tblAllergy.AllergyDescription_str, tblPersonAllergy.pers' +
        'onAllergySource_Str'
      
        'FROM tblPersonAllergy INNER JOIN tblAllergy ON tblAllergy.Allerg' +
        'yCode_Str = tblPersonAllergy.personAllergyDescription_Str'
      'WHERE (person_ID = :person_ID)'
      'ORDER BY tblAllergy.AllergyDescription_str')
    Left = 296
    Top = 161
    object qryPersonAllergiesAllergyDescription_str: TWideStringField
      FieldName = 'AllergyDescription_str'
      Size = 150
    end
    object qryPersonAllergiespersonAllergySource_Str: TWideStringField
      FieldName = 'personAllergySource_Str'
      Size = 50
    end
  end
  object qryPatientProtocols: TADOQuery
    Connection = conPatientPrescription
    CursorType = ctStatic
    DataSource = dsqryPatientList
    Parameters = <
      item
        Name = 'person_ID'
        DataType = ftGuid
        NumericScale = 255
        Precision = 255
        Size = 16
        Value = Null
      end>
    SQL.Strings = (
      
        'SELECT DISTINCT TblStdProtocol.Name_Str AS Protocol, TblStdProto' +
        'col.Indication_str AS Indication, TblPrescription.Date_dat, TblP' +
        'rescription.prescriptionStatus_Str AS Status'
      'FROM  TblStdProtocol INNER JOIN TblRXItem ON '
      '     TblStdProtocol.Protocol_ID = TblRXItem.FrmProtocol_ID'
      '      INNER JOIN TblPrescription ON '
      '     (TblRXItem.RX_ID = TblPrescription.prescription_ID)'
      '     INNER JOIN tblPerson ON'
      '     (tblPerson.person_ID = TblPrescription.patient_ID)'
      
        'WHERE (tblPerson.person_ID=:person_ID) AND (TblPrescription.pres' +
        'criptionStatus_Str NOT LIKE '#39'**VOID%'#39')'
      
        'GROUP BY TblStdProtocol.Name_Str, TblStdProtocol.Indication_str,' +
        ' TblPrescription.Date_dat, TblPrescription.prescriptionStatus_St' +
        'r'
      'ORDER BY Date_dat DESC')
    Left = 296
    Top = 192
    object qryPatientProtocolsProtocol: TWideStringField
      FieldName = 'Protocol'
      Size = 100
    end
    object qryPatientProtocolsIndication: TWideStringField
      FieldName = 'Indication'
      Size = 350
    end
    object qryPatientProtocolsDate_dat: TDateTimeField
      FieldName = 'Date_dat'
    end
    object qryPatientProtocolsStatus: TWideStringField
      FieldName = 'Status'
      Size = 50
    end
  end
  object dsPatientProtocols: TDataSource
    DataSet = qryPatientProtocols
    Left = 336
    Top = 192
  end
  object qryPatientHopping: TADOQuery
    Connection = conPatientPrescription
    Parameters = <
      item
        Name = 'PatientID'
        Attributes = [paNullable]
        DataType = ftGuid
        NumericScale = 255
        Precision = 255
        Size = 16
        Value = Null
      end>
    SQL.Strings = (
      'SELECT TOP 5 ConsultingRoom_str, Date_dat'
      'FROM tblHealthCareRegister'
      'WHERE Patient_ID = :PatientID'
      'ORDER BY Date_dat DESC')
    Left = 296
    Top = 240
  end
  object qryWasMedicationReceived: TADOQuery
    Connection = conPatientPrescription
    DataSource = dsqryPatientList
    Parameters = <
      item
        Name = 'Person_ID'
        Attributes = [paNullable]
        DataType = ftGuid
        NumericScale = 255
        Precision = 255
        Size = 16
        Value = Null
      end
      item
        Name = '@StartDate'
        Attributes = [paNullable]
        DataType = ftDateTime
        NumericScale = 3
        Precision = 23
        Size = 16
        Value = Null
      end
      item
        Name = '@EndDate'
        Attributes = [paNullable]
        DataType = ftDateTime
        NumericScale = 3
        Precision = 23
        Size = 16
        Value = Null
      end>
    SQL.Strings = (
      'SELECT DISTINCT TblRXItem.FrmFormulation_Str'
      
        'FROM TblRXItem INNER JOIN TblPrescription ON TblRXItem.RX_ID = T' +
        'blPrescription.prescription_ID'
      'WHERE TblPrescription.Patient_ID=:Person_ID'
      'AND (TblPrescription.Date_dat >=:@StartDate)'
      'AND (TblPrescription.Date_dat <=:@EndDate)'
      'AND TblPrescription.dispensingPosted_bol = 1')
    Left = 296
    Top = 272
  end
end
