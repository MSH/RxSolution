object DemanderLookupFrm: TDemanderLookupFrm
  Left = 189
  Top = 64
  BorderIcons = []
  BorderStyle = bsToolWindow
  Caption = 'DemanderLookupFrm'
  ClientHeight = 536
  ClientWidth = 615
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  Position = poMainFormCenter
  PixelsPerInch = 96
  TextHeight = 13
  object pnlRequisitionListBackground: TPanel
    Left = 0
    Top = 0
    Width = 615
    Height = 536
    Align = alClient
    BevelOuter = bvNone
    Color = clWindow
    TabOrder = 0
    object shpRequisitionList: TShape
      Left = 5
      Top = 5
      Width = 604
      Height = 508
      Brush.Color = clWindow
      Pen.Color = clHighlight
      Pen.Style = psInsideFrame
    end
    object lblMainFormTitle: TLMDSimpleLabel
      Left = 10
      Top = 10
      Width = 591
      Height = 32
      Alignment = agCenter
      AutoSize = False
      Color = 16311512
      Font.Charset = ANSI_CHARSET
      Font.Color = clBackground
      Font.Height = -21
      Font.Name = 'Haettenschweiler'
      Font.Style = []
      Font3D.DisabledStyle = dsNone
      Font3D.LightColor = clBtnText
      Font3D.ShadowColor = clWindow
      Font3D.Style = tdSunkenShadow
      ParentColor = False
      ParentFont = False
      Transparent = False
      Caption = 'S e l e c t    D e m a n d e r'
      Options = []
    end
    object pnlToolBar: TPanel
      Left = 10
      Top = 45
      Width = 591
      Height = 26
      BevelOuter = bvNone
      Color = clWindow
      TabOrder = 2
      object btnLast: TElSpeedButton
        Left = 120
        Top = 0
        Width = 24
        Height = 25
        PullDownBtn = mbLeft
        Flat = True
        NormalImage.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFDE9D3BFBFD4E9FBFDFEFEFEFFFFFFFF}
        DisabledImage.Data = {
          
          746D6D747D838484838484848484}
        MouseInImage.Data = {
          
          FFFFFFFFFFFFFDFDFDFAB6353535353538B5B5FAFDFDFFFFFFFF}
        PressedImage.Data = {
          
          FFFFFFFEFEFEFEFEFEFEF9CF674D547CD0F1F8FCFEFEFEFFFFFF}
        AutoSize = False
        TransparentColor = clBlack
        Align = alLeft
        Color = clBtnFace
        ParentColor = False
        DockOrientation = doNoOrient
      end
      object btnPageDown: TElSpeedButton
        Left = 96
        Top = 0
        Width = 24
        Height = 25
        Hint = 'Page down'
        PullDownBtn = mbLeft
        Flat = True
        NormalImage.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFDE8D2BEBED4E8FBFDFEFEFEFFFFFFFF}
        DisabledImage.Data = {
          }
        MouseInImage.Data = {
          
          FFFFFFFFFFFFFDFDFDFAB336363636365254B3EFFDFDFDFFFFFF}
        PressedImage.Data = {
          
          FFFFFFFEFEFEFEFEFEFEF9C9674E537DCAE9F8FCFEFEFEFFFFFF}
        AutoSize = False
        TransparentColor = clBlack
        Align = alLeft
        Color = clBtnFace
        ParentColor = False
        DockOrientation = doNoOrient
      end
      object btnPageUp: TElSpeedButton
        Left = 24
        Top = 0
        Width = 24
        Height = 25
        Hint = 'Page up'
        PullDownBtn = mbLeft
        Flat = True
        NormalImage.Data = {
          
          FFFFFFFFFFFFFEFEFFFFFDE8D1BABAD2E8FAFDFEFEFEFFFFFFFF}
        DisabledImage.Data = {
          
          818282828282}
        MouseInImage.Data = {
          
          FFFFFFFFFFFFFDFDFAF8AF37373737373AAEAEF8FDFDFDFFFFFF}
        PressedImage.Data = {
          
          FEFFFFFEFEFEFEFEFEFEF9C74D484D7BC8F0F7FCFEFEFEFEFFFF}
        AutoSize = False
        TransparentColor = clBlack
        Align = alLeft
        Color = clBtnFace
        ParentColor = False
        DockOrientation = doNoOrient
      end
      object btnFirst: TElSpeedButton
        Left = 0
        Top = 0
        Width = 24
        Height = 25
        PullDownBtn = mbLeft
        Flat = True
        NormalImage.Data = {
          
          FFFFFFFFFFFFFFFFFFFFFDE9D5C1C1D6E9FBFDFEFEFEFFFFFFFF}
        DisabledImage.Data = {
          
          746C6C747D838484838484848484}
        MouseInImage.Data = {
          
          FFFFFFFFFFFFFDFDFAF8AF37373737373AAEAEF8FDFDFDFFFFFF}
        PressedImage.Data = {
          
          FEFFFFFEFEFEFEFEFEFEF9C74D484D7BC8F0F7FCFEFEFEFEFFFF}
        TransparentColor = clBlack
        Align = alLeft
        Color = clBtnFace
        ParentColor = False
        ParentShowHint = False
        ShowHint = True
        DockOrientation = doNoOrient
      end
      object pnlToolBar_Line1: TShape
        Left = 0
        Top = 25
        Width = 591
        Height = 1
        Align = alBottom
        Pen.Color = clHighlight
      end
      object btnPrior: TElSpeedButton
        Left = 48
        Top = 0
        Width = 24
        Height = 25
        PullDownBtn = mbLeft
        Flat = True
        NormalImage.Data = {
          
          FFFFFFFFFFFFFFFFFEFEFEF0D1B3B3D1F0FDFEFEFFFFFFFFFFFF}
        DisabledImage.Data = {
          
          6C727373737373737373}
        MouseInImage.Data = {
          
          FFFFFFFFFFFDFDFAF8F0EF563737373756EFF0F0FAFDFDFDFFFF}
        PressedImage.Data = {
          
          FFFFFFFFFFFFFFFDFCFCFADCAD5A5AADDCFAFAFAFDFEFFFFFFFF}
        AutoSize = False
        TransparentColor = clBlack
        Align = alLeft
        Color = clBtnFace
        ParentColor = False
        DockOrientation = doNoOrient
      end
      object btnNext: TElSpeedButton
        Left = 72
        Top = 0
        Width = 24
        Height = 25
        PullDownBtn = mbLeft
        Flat = True
        NormalImage.Data = {
          
          FFFFFFFFFFFFFFFFFEFEFEEECDAFAFCDEEFDFEFEFFFFFFFFFFFF}
        DisabledImage.Data = {
          }
        MouseInImage.Data = {
          
          FFFFFFFFFFFDFDF9F7EDEC563636363656ECEDEDF9FDFDFDFFFF}
        PressedImage.Data = {
          
          FFFFFFFFFFFFFFFDFCFCFADDB35959B3DDF9FAFAFDFEFFFFFFFF}
        AutoSize = False
        TransparentColor = clBlack
        Align = alLeft
        Color = clBtnFace
        ParentColor = False
        DockOrientation = doNoOrient
      end
      object btnFilter: TElSpeedButton
        Left = 151
        Top = 0
        Width = 24
        Height = 25
        Hint = 'Select product filter'
        PullDownBtn = mbLeft
        PullDownInterval = 0
        PullDownEnabled = True
        Flat = True
        NormalImage.Data = {
          
          0000000000000000000000070B11110B07000000000000000000}
        DisabledImage.Data = {
          
          0000}
        MouseInImage.Data = {
          }
        PressedImage.Data = {
          
          0000}
        AutoSize = False
        TransparentColor = clBlack
        Align = alLeft
        Color = clBtnFace
        ParentColor = False
        DockOrientation = doNoOrient
      end
      object pnlToolBar_N1: TPanel
        Left = 144
        Top = 0
        Width = 7
        Height = 25
        Align = alLeft
        BevelOuter = bvNone
        Color = clWindow
        TabOrder = 0
      end
      object cbCodeChange: TComboBox
        Left = 440
        Top = 0
        Width = 153
        Height = 21
        Hint = 'Display selected code in the grid'
        ItemHeight = 13
        TabOrder = 1
        Text = 'cbCodeChange'
        Items.Strings = (
          'Product Code'
          'ICN Code'
          'NSN Code'
          'ECN Code')
      end
    end
    object dbgProductLookup: TDBGrid
      Left = 10
      Top = 96
      Width = 591
      Height = 409
      Hint = 
        'Select product. To select more than one product, hold down the [' +
        'control] key and click.'
      BorderStyle = bsNone
      DataSource = MainDm.dstblGeneralProductDescription
      FixedColor = clInfoBk
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      Options = [dgTitles, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit, dgMultiSelect]
      ParentFont = False
      TabOrder = 3
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clHighlight
      TitleFont.Height = -11
      TitleFont.Name = 'Tahoma'
      TitleFont.Style = []
      Columns = <
        item
          Expanded = False
          FieldName = 'ProductCode_str'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clHighlight
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = [fsBold]
          Title.Caption = 'Code'
          Width = 65
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'Description'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clHighlight
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          Width = 505
          Visible = True
        end>
    end
    object btnSelect: TBitBtn
      Left = 8
      Top = 520
      Width = 75
      Height = 25
      Hint = 'Accept and add selected products'
      Caption = '&Select'
      TabOrder = 4
      Kind = bkOK
    end
    object btnCancel: TBitBtn
      Left = 88
      Top = 520
      Width = 75
      Height = 25
      Hint = 'Cancel selection and close this forrm'
      Caption = '&Cancel'
      TabOrder = 5
      Kind = bkCancel
    end
    object edtCodeSearch: TEdit
      Left = 8
      Top = 72
      Width = 145
      Height = 21
      Hint = 'Search for a product by typing in the code'
      Color = clInfoBk
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clTeal
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
    end
    object edtDescriptionSearch: TEdit
      Left = 160
      Top = 72
      Width = 441
      Height = 21
      Hint = 'Search for a product by typing in the description'
      Color = clInfoBk
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clTeal
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
    end
  end
end
