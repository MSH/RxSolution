...
//    function PostItems( pItems : TADOStoredProc): Boolean;
    ...
    function CheckAllTotalBatches: Boolean;
...
function TReceiptDm.CheckAllTotalBatches;
var
...
procedure TReceiptDm.CreateTextGRVFile;
var
 F1, F2  : TextFile;
 DataStr : String;
...
begin               
  ...
  with stpStockControlReceipt do
   ...
   if (State in [dsEdit, dsInsert]) then
    Post;
   ReceivedBy         := CodeMyString(Copy(FieldByName('ReceivedBy_str').AsString, 0, Pos('(', FieldByName('ReceivedBy_str').AsString) - 1), 50);
   ReceivedDate       := FormatDateTime('dd/mm/yyyy', FieldByName('Ordered_dat').AsDateTime) + ' ';
   OrderRef           := Copy(Trim(FieldByName('OrderNo_str').AsString), Length(FieldByName('OrderNo_str').AsString)-4, 5);
   ReceiptRef         := Copy(Trim(FieldByName('ReceiptNo_str').AsString), Length(FieldByName('ReceiptNo_str').AsString)-4, 5); 
...
//  MessageDlg('There are products with blank NSN numbers in the order.', mtWarning,[mbOk], 0);
 ...
 with stpReceiptListReport do //need parameters
  begin
  Close;
  Parameters.ParamByName('@RecID').Value := FReceiptNo;
  Open;
  First;
   ...
   while not Eof do
    begin
      ...
      //1234567812345678123456789012345612345678912345678901234YY12-5012345678
        ...
        Supplier_Code      := CodeMyString(Copy(Trim(FieldByName('Code_str').AsString), 1, 8), 8);   //supplier Code   ? Length
        Supplier_Name      := CodeMyString(Copy(Trim(FieldByName('Name_str').AsString), 1, 8), 8);   //supplier Name   ? Length
        ...
        DataStr := Supplier_Code + Dem_Code + Local_Dem_No + Objective_Code +
          NSN_str + Demand_Qty + FMS_Code +  Recurr_Ind + Use_Loc_Dem_No + ReceivedBy + ReceivedDate {+ AuthorisedBy + AuthorisedDate};
        Writeln(F2, DataStr) ;
      end;
      Next;
    end;
   CloseFile(F2);
   MessageDlg('PMPU Receipt file '+SaveDialog1.FileName+' has been created successfully', mtInformation, [mbOk], 0);
