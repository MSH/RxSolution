...
type
  TReceiptDetailFrm = class(TForm)
    ...
    N3: TMenuItem;
    atnCreatePMPUTextFile: TAction;
    GRVTextFile1: TMenuItem;
...
//    procedure tbsReceiptDisplayResize(Sender: TObject);
    ...
    procedure atnCreatePMPUTextFileExecute(Sender: TObject);
  private
...
{if not (ReceiptDm.stpStockControlReceiptItems.State in [dsEdit, dsInsert]) then ReceiptDm.stpStockControlReceiptItems.Edit;
 ...
 if ReceiptDm.stpStockControlReceipt.State in [dsEdit, dsInsert] then ReceiptDm.stpStockControlReceipt.Post;
   ...
   ReceiptDm.CheckAllTotalBatches(False); 
...
begin
 ...
 ReceiptDm.CheckAllTotalBatches(True);

end;

procedure TReceiptDetailFrm.atnCreatePMPUTextFileExecute(Sender: TObject);
begin

 with ReceiptDm do
  begin
  CreateTextFileReceivingPMPUReport;
  end;
