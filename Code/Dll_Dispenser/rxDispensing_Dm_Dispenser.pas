unit rxDispensing_Dm_Dispenser;
...
const
  ...
  CURRENTPERSONTYPE = AREA_DISPENSER;
  CURRENTPERSONAREA = AREA_DISPENSER_DESC;


type
  TdmPerson = class(TDataModule)
    conPerson: TADOConnection;
  ...
  if FDatabase <> '' then
    ...
    with AddQry do
     ...
     if doDelete then
        begin
        AddQry.Close;
        AddQry.SQL.Add('SELECT tblPerson.person_ID  FROM tblPerson INNER JOIN TblPrescription ON tblPerson.person_ID = TblPrescription.patient_ID where tblPerson.person_ID = ''' + ID + '''');
        AddQry.Open;

         if AddQry.RecordCount  = 0 then
           begin
           AddQry.Close;
           ...
           ShowMessage('Cannot delete dispenser with active prescriptions');
...
begin
  ...
  with Dataset do
    ...
    prvActive_bol := FieldByName('personActive_bol').AsBoolean;
    ...
    FieldByName('personDispenser_bol').AsBoolean := True;
//    FieldByName('personPrescriber_bol').AsBoolean := False;
    FieldByName('personPatient_bol').AsBoolean := False;

    if ((qryDemanders.RecordCount > 0) and (FieldByName('personInstitution_str').AsString <> '')) then
     FieldByName('personInstitution_ID').AsString := qryDemanders.FieldByName('DemanderUnique_ID').AsString;
    FieldByName('personActive_bol').AsBoolean :=  prvActive_bol;
...
begin
  ...
  FDatabase := Value;
  conPerson.ConnectionString := FDatabase;
  SetDatabaseLinks;
...
begin
  ...
  if (FDatabase <> '') and (FPerson <> '') then
    ...
    ConnectDataSource(FDatabase, conPerson);
    DoDefaultChecks;
    end;
...
begin
  ...
  with qryPersonDetail do
    if Active then
      begin
      if not (State in [dsEdit, dsInsert]) then Edit;
//SM      FieldByName('personActive_Bol').AsBoolean := activePresc; //Do not look at when position starts or end as it not relavent to the dispenser//SM
